...
# TODO change fsa_object to fsa_schema
# TODO add depreciation warning before release but do not remove!
# TODO: need to remove the use_pubchem flag as pubchem will be provided as AgentExecutor
def create_linqx_chat_agent(
    ...
    # TODO figure out a better way to add additional instructions in a modular manner
    ...
    # TODO potentially add info here for embedding tools
    ...
    #? Do we want to add the pubchem tools as a paramater to be set for usage in the inititation of the agent or should they be present by default in the agent?
    ...
    if rag_vectordb_path: 
        ...
        def call_RAG_agent(question: str) -> str:
            ...
            TAKSK: You have to frame the best possible "question" that is extremely descriptive and then use it as a parameter to query the relevant sources of information.
            """
    ...
    if use_pubchem:
        @tool
        def call_pubchem_agent(question: str) -> str:
            """
            This function is used when there is a need to query the PubChem database for information.
            the following tools are available:
            get_sids_from_cid: This tool will return the substance id from the compound id.
            get_cids_from_sid: This tool will return the compound id from the substance id.
            get_synonym: This tool will return the synonyms of the chemical compound.
            get_description: This tool will return the description of the chemical compound.
            get_assay_description: This tool will return the description of the assay.
            get_assay_id_from_smiles: This tool will return the assay id from the smiles.
            get_assay_name_from_aid: This tool will return the assay name from the assay id.
            """
            pubchem_agent_instance = pubchem_agent(question)
            store_output = pubchem_agent_instance.invoke({"question": question})
            # print(store_output)
            return store_output['output']
        
        tools.append(call_pubchem_agent)
    ...
    # Assign agent memory
    # TODO redo memory assignment here to be more modular
    # TODO rename intermediate memory to state memory
...
"""
class LinqxMicroserviceAgent(BaseModelV2):
    '''
    '''
    # Core attributes
    microservice: BaseDriverMicroservice
    llm: BaseLanguageModel = ChatOpenAI(temperature=0)
    prompt_template: BasePromptTemplate = BASE_LINQX_CHAT_PROMPT_TEMPLATE
    ...
    #FSA attributes
    fsa_schema: Type[BaseModelV1] | None = None
    start_state: BaseModelV1 | None = None
"""